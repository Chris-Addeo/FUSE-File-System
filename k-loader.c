                                ...
                                x86_64_pagetable *(*allocator)(void))
{
    ...
    // allocate memory
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) // Original has (PTE_P | PTE_W | PTE_U) for the perms
    {
        if (assign_physical_page(addr, p->p_pid) < 0 || virtual_memory_map(p->p_pagetable, addr, addr, PAGESIZE,
    ...
    // Get the permissions ------ Step 6 Extra Credit
    int perms = (PTE_P | PTE_U);
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0)
    {
        // Loop through the memory and assign the new permissions
        for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE)
        {
            // Map with correct permissions
            vamapping vam = virtual_memory_lookup(p->p_pagetable, addr);
            virtual_memory_map(p->p_pagetable, va, vam.pa, PAGESIZE, perms, NULL);
        }
    }
